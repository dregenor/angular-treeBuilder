...
angular.module('tree-builder.services')
    .factory('tree-builder.services.builder',function(){
        ...
        builder.prototype.replaceNode = function(node,target){
            ...
            if(node.id && this.items[node.id]){
                // нода уже есть и у нее наверно даже чилды есть
                itm = this.items[node.id];
                // отцепить от родителя
                if(itm._parent){
                    itm._parent.removeChild(itm);
                }
            } else {
            ...
            if(parent instanceof TreeNode){
               parent.replaceChild(target,itm);
         ...
         * 
         * @param obj
         * @param {Function} checkCb
         */
        builder.prototype.makeFinishNode = function(obj,checkCb){
            
            Object.keys(this.items).forEach(function(id){
                while (checkCb(this.items[id])){
                    var node = angular.extend({id:this.getNextId()},angular.copy(obj)),    
                        lastNode = TreeNode.create(node,node.id);
                    lastNode.setParent(this.items[id]);
                    this.items[lastNode.id] = lastNode;
                }
            }.bind(this))
